...
from server.shared.helpers.utils.contour_finder import find_contours, arrange_boxes
...
def get_max_hw(point):
    ...
    (tl, tr, br, bl) = rect
    width_x = np.sqrt(((br[0] - bl[0]) ** 2) + ((br[1] - bl[1]) ** 2))
    width_y = np.sqrt(((tr[0] - tl[0]) ** 2) + ((tr[1] - tl[1]) ** 2))
    max_width = max(int(width_x), int(width_y))
    height_x = np.sqrt(((tr[0] - br[0]) ** 2) + ((tr[1] - br[1]) ** 2))
    height_b = np.sqrt(((tl[0] - bl[0]) ** 2) + ((tl[1] - bl[1]) ** 2))
    max_height = max(int(height_x), int(height_b))

    return max_width, max_height


def get_transform_matrix(point, max_width, max_height):
...
def make_warp_img(img, mask_img):
    ...
    max_width, max_height = get_max_hw(warped_point)
    matrix = get_transform_matrix(warped_point, max_width, max_height)
